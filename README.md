# CalculadoraIncertezaEC
Aplicação web desenvolvida em Python para cálculo do estoque de carbono no solo e da incerteza associada, utilizando parâmetros provenientes de análises de amostras de solo. A ferramenta automatiza os cálculos a partir de modelos matemáticos e disponibiliza os resultados em um dashboard interativo, facilitando a interpretação dos dados e a tomada de decisão em contextos de monitoramento ambiental e manejo agrícola. O projeto foi desenvolvido como prova de conceito (MVP) e disponibilizado como aplicação web utilizando Streamlit.

A aplicação pode ser acessada online: https://calculadora-incertezaec.streamlit.app/

Funcionalidades:
-Cálculo automático do estoque de carbono no solo
-Cálculo da incerteza associada às medições
-Interface web interativa e intuitiva
-Automação de cálculos que antes eram realizados em planilhas
-Dashboard acessível via navegador
-Versionamento do projeto com Git/GitHub

Tecnologias utilizadas:
Python;
Streamlit;
HTML;
CSS;
Git;
GitHub;
Streamlit Cloud (deploy).

Metodologia:
O desenvolvimento do projeto seguiu a abordagem de MVP (Minimum Viable Product), permitindo a construção de uma solução funcional em ciclos curtos de desenvolvimento e validação.

As principais etapas foram:
-Levantamento dos requisitos e parâmetros de cálculo
-Modelagem matemática do cálculo do estoque de carbono e propagação de incertezas
-Implementação das funções em Python
-Desenvolvimento da interface web com Streamlit
-Versionamento do código no GitHub
-Deploy da aplicação utilizando Streamlit Cloud
-Testes e validação com dados reais

Contexto do projeto:
Este projeto foi desenvolvido como Trabalho de Conclusão de Curso (TCC) do curso de Análise e Desenvolvimento de Sistemas, com o objetivo de transformar uma planilha de cálculo em uma aplicação web acessível e automatizada.
A ferramenta foi testada utilizando dados reais de análises de solo, demonstrando viabilidade para utilização em contextos relacionados à agricultura de precisão e monitoramento ambiental.

Possíveis melhorias futuras:
-Inclusão de gráficos para visualização do estoque de carbono
-Comparação entre diferentes tipos de solo
-Exportação dos resultados em PDF ou Excel
-Ampliação da aplicação para outros protocolos de amostragem

Autora:
Maisa Cristina Lázaro Dallilo
Doutora em Química Analítica
MBA em Data Science and Analytics
Graduanda em Análise e Desenvolvimento de Sistemas
